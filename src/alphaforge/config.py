...
All randomness flows from `seed`; all paths flow from `AlphaforgeConfig` so a
...
class AlphaforgeConfig(BaseModel):
    seed: int = 1
    run_dir: Path = Field(default=Path("runs"))
    db_path: Path = Field(default=Path("runs/alphaforge.sqlite"))
    n_experiments: int = 8
    max_parallel: int = 4
